...
interface DeleteRecurringBillModalProps {
  isOpen: boolean;
  onClose: () => void;
  onDeleteRecurringBill: (transaction: RecurringBill) => void;
  selectedRecurringBill: RecurringBill | null;
}

const DeleteRecurringBillModal = ({
  isOpen,
  onClose,
          ...
          <div className="flex w-full flex-col gap-5">
            <div className="flex w-full items-center justify-between gap-10">
              <h2 className="text-xl font-bold leading-[1.2] text-grey-900">
                Delete '{selectedRecurringBill?.name}'?
              </h2>
              <button
                onClick={onClose}
            ...
            <button
              onClick={() => {
                if (selectedRecurringBill) {
                  onDeleteRecurringBill(selectedRecurringBill);
                }
              }}
...
export default DeleteRecurringBillModal;
